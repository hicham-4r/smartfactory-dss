...
class RequestContextMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        try:
            host_error = self._host_error(request, request_id)
            if host_error is not None:
                status_code = host_error.status_code
                return self._secure(host_error, request_id)

            size_error = self._size_error(request, request_id)
            if size_error is not None:
                status_code = size_error.status_code
                return self._secure(size_error, request_id)
    ...
    def _host_error(self, request: Request, request_id: str) -> JSONResponse | None:
        host = request.url.hostname
        if host is None or not self._host_allowed(host.lower()):
            return JSONResponse(
                status_code=400,
    ...
    def _size_error(self, request: Request, request_id: str) -> JSONResponse | None:
